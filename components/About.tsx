...
export default function About() {
  return (
    <div
      id="about"
      className="relative flex flex-col gap-4 p-8 justify-center items-center min-h-screen w-full overflow-hidden bg-black-100"
    >
      <Spotlight
      ...
      <div className="grid grid-cols-[4fr,6fr,2fr] gap-4 md:flex-row justify-center max-w-screen-lg">
        <motion.div initial={{ opacity: 0 }}
          ...
          viewport={{ amount: 1 }} className="p-4 rounded-xl bg-black/20 backdrop-blur-3xl border">
          <img
        ...
        <motion.ul
          ...
          whileInView={{ opacity: 1, transition: { staggerChildren: 0.5 } }}
          viewport={{ amount: 1 }}
          className="flex flex-col items-center justify-center gap-4 rounded-xl bg-black/20 backdrop-blur-3xl border"
        >
          {Socials.map((item) => (
            <motion.li
              key={item.name}
              ...
              whileInView={{ opacity: 1 }}
              viewport={{ amount: 1 }}
              className="flex items-center hover:text-neutral-400"
            >
              <Link href={item.url}>
                <item.icon size="32" />
              </Link>
        ...
        >
          <h2 className="text-xl uppercase text-neutral-50">an aspiring full stack developer</h2>
